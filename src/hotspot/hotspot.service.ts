...
export class HotspotService {
  ...
  async getLoadMap(organisation: string, filter: HotspotFilterDto): Promise<HotspotDto[]> {
    const [ loadResults, locatiionResults ] = await Promise.all([await this.carrierService.searchLoad(
      organisation,
      filter,
    ),
    await this.carrierService.searchLocation(
      organisation,
      filter,
    )]);
  ...
  async getIdleMap(organisation: string, filter: HotspotFilterDto): Promise<HotspotDto[]> {
    const [ idleResults, locatiionResults ] = await Promise.all([await this.carrierService.searchIdle(
      organisation,
      filter,
    ),
    await this.carrierService.searchLocation(
